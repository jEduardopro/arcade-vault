...
// The real CAÍDA cartridge: the canvas plus the glue that turns what the engine
// reports into React state. Everything around it — HUD, pause overlay, end
// modal — belongs to <PlayerShell>.
//
// Same shape as asteroides-game.tsx, because the boundary SPEC 05 drew is the
// same one: the engine lives in a ref, not in state, since it is a mutable
// object with a lifetime tied to the mount and re-rendering must never build a
// second one.
...
import {
    ...
    type GameSnapshot,
    type GameStatus,
} from "@/app/lib/engines/caida/engine";
import type { Game } from "@/app/lib/games";

// lives is 0 for the whole run: CAÍDA has no lives, and the shell renders "—".
const INITIAL: GameSnapshot = { score: 0, lives: 0, level: 1, lines: 0 };
...
export default function CaidaGame({ game }: { game: Game }) {
    ...
    return (
        ...
        >
            <canvas
                ...
                aria-label={`${game.title}: juego de piezas que caen en canvas, se controla con el teclado`}
            />
