...
// The real Asteroides cartridge: the canvas plus the glue that turns what the
// engine reports into React state. Everything around it — HUD, pause overlay,
// end modal — belongs to <PlayerShell>.
//
// The engine lives in a ref, not in state: it is a mutable object with a
// lifetime tied to the mount, and re-rendering must never build a second one.
...
import { RUN } from "@/app/lib/engines/asteroides/constants";
import {
    createAsteroidsEngine,
    type EngineHandle,
    type GameSnapshot,
    type GameStatus,
} from "@/app/lib/engines/asteroides/engine";
...
export default function AsteroidesGame({ game }: { game: Game }) {
    ...
    return (
        <PlayerShell
            ...
            extraStat={
                snapshot.tripleShot > 0
                    ? {
                          label: "3x",
                          value: `${snapshot.tripleShot.toFixed(1)}s`,
                      }
                    : null
            }
        ...
        >
            <canvas
                ...
                aria-label={`${game.title}: juego de acción en canvas, se controla con el teclado`}
            />
